...
#define MYPRINTF(format, args...)	fprintf(stderr, format, ## args)
...
int open (const char *pathname, int flags, ...){
	...
	if (strcmp (pathname, spy_file)){	
		...
		MYPRINTF ("HOOK: opened file %s (fd=%d)\n", pathname, fd);
		return fd;
	}
...
/*
	if (fd == hook_fd) {
		MYPRINTF ("HOOK: closed hooked file %s (fd=%d)\n", spy_file, fd);
	} else {
		MYPRINTF ("HOOK: closed file descriptor (fd=%d)\n", fd);
	}
*/
		
	retval = func_close (fd);
	if (retval)
		MYPRINTF("HOOK: closed file descriptor (fd=%d), ret: %d, %m\n", fd, retval);
	return retval;
}
